// farmer-noauth.test.js — Selenium test TC-FARMER-02
const { Builder, By, until, Key } = require('selenium-webdriver');
const fs = require('fs');

(async function TC_FARMER_02() {
  ...
  try {
    ...
    fs.writeFileSync('farmer-noauth-result.png', png, 'base64');
    console.log('Saved screenshot: farmer-noauth-result.png');

    if (!urlAfter.includes('/dashboard/farmer') && !urlAfter.includes('/farmer')) {
      console.log('TC-FARMER-02: FAIL (expected) — buyer correctly blocked from farmer dashboard (redirected to ' + urlAfter + ')');
    } else {
      console.log('TC-FARMER-02: UNEXPECTED — buyer reached farmer dashboard!');
    }
  } catch (err) {
    console.log('TC-FARMER-02: FAIL — error:', err.message);
  } finally {
